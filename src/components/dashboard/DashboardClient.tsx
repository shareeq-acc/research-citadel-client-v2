...
function VaultSettingsTab({ activeVault, editVaultName, setEditVaultName, editVaultDesc, setEditVaultDesc, editVaultPrivacy, setEditVaultPrivacy, updatingVaultSettings, handleUpdateVaultSettings, confirmExitVault, setConfirmExitVault, handleExitVault, confirmDeleteVault, setConfirmDeleteVault, handleDeleteVault, settingsSuccessMsg, settingsErrorMsg, handleMuteVaultToggle, mutedVaults, activeSettingsSubTab, setActiveSettingsSubTab, simulateCollaborators, setSimulateCollaborators, expandAnnotationsByDefault, setExpandAnnotationsByDefault, notifyOnNewAnnotations, setNotifyOnNewAnnotations, onNavigateToPassport, idCardNickname, idCardSpecialization, idCardMotto }: any) {
  return (
    <div className="bg-white rounded-sm border-4 border-neo-dark p-5 shadow-[4px_4px_0px_#000] space-y-4 text-left">
      <h3 className="font-display font-black text-sm uppercase tracking-wider text-neo-dark border-b-2 border-stone-200 pb-2">Vault Settings</h3>
      
      {settingsSuccessMsg && <div className="p-3 bg-emerald-50 border-2 border-emerald-500 text-xs font-mono font-bold text-emerald-700 rounded-sm">✓ {settingsSuccessMsg}</div>}
      {settingsErrorMsg && <div className="p-3 bg-rose-50 border-2 border-rose-500 text-xs font-mono font-bold text-rose-700 rounded-sm">⚠️ {settingsErrorMsg}</div>}
      ...
      <form onSubmit={handleUpdateVaultSettings} className="space-y-4">
        <div>
          ...
          <input type="text" required value={editVaultName} onChange={(e) => setEditVaultName(e.target.value)} className="w-full text-xs p-2 border-2 border-neo-dark rounded-sm focus:outline-none" />
        </div>
        <div>
          <label className="block text-[9px] font-bold font-mono text-stone-600 uppercase mb-1">Description</label>
          <textarea value={editVaultDesc} onChange={(e) => setEditVaultDesc(e.target.value)} className="w-full text-xs p-2 border-2 border-neo-dark rounded-sm focus:outline-none h-16 resize-none" />
        </div>
        <div>
          <label className="block text-[9px] font-bold font-mono text-stone-600 uppercase mb-1">Privacy</label>
          <div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={() => setEditVaultPrivacy("PRIVATE")} className={`py-1.5 px-3 border border-neo-dark font-display font-bold text-[10px] rounded-sm cursor-pointer ${editVaultPrivacy === "PRIVATE" ? "bg-neo-yellow" : "bg-stone-50"}`}>Private</button>
            <button type="button" onClick={() => setEditVaultPrivacy("PUBLIC")} className={`py-1.5 px-3 border border-neo-dark font-display font-bold text-[10px] rounded-sm cursor-pointer ${editVaultPrivacy === "PUBLIC" ? "bg-neo-yellow" : "bg-stone-50"}`}>Public</button>
          </div>
        </div>
        <button type="submit" disabled={updatingVaultSettings} className="neo-btn px-4 py-2 text-xs">
          {updatingVaultSettings ? "Saving..." : "Save Settings"}
        </button>
      </form>

      <div className="border-t-2 border-dashed border-stone-200 pt-4 space-y-3">
        <h4 className="font-display font-black text-xs uppercase text-stone-500">Preferences</h4>
        <label className="flex items-center gap-2 cursor-pointer select-none text-xs font-mono">
          <input type="checkbox" checked={mutedVaults[activeVault.id] || false} onChange={() => handleMuteVaultToggle(activeVault.id)} className="form-checkbox border-2 border-neo-dark" />
          Mute notifications for this vault
        </label>
        <label className="flex items-center gap-2 cursor-pointer select-none text-xs font-mono">
          <input type="checkbox" checked={simulateCollaborators} onChange={(e) => setSimulateCollaborators(e.target.checked)} className="form-checkbox border-2 border-neo-dark" />
          Simulate mock collaborators
        </label>
      </div>

      <div className="border-t-2 border-dashed border-stone-200 pt-4 space-y-3">
        <h4 className="font-display font-black text-xs uppercase text-stone-500">ID Card / Passport</h4>
        <button type="button" onClick={onNavigateToPassport} className="w-full py-2 px-4 text-xs font-display font-black border-2 border-neo-dark bg-stone-50 hover:bg-stone-100 cursor-pointer rounded-sm text-neo-dark">Open Identity Credentials Compiler</button>
      </div>

      {activeVault.myRole === "OWNER" && (
        <div className="border-t-2 border-dashed border-rose-200 pt-4 space-y-3">
          <h4 className="font-display font-black text-xs uppercase text-rose-500">Danger Zone</h4>
          {!confirmDeleteVault ? (
            <button type="button" onClick={() => setConfirmDeleteVault(true)} className="w-full py-2 px-4 text-xs font-display font-black border-2 border-rose-500 bg-rose-50 hover:bg-rose-100 cursor-pointer rounded-sm text-rose-700">Delete Vault</button>
          ) : (
            <div className="p-3 bg-rose-50 border-2 border-rose-500 rounded-sm space-y-2">
              <p className="text-xs font-mono font-bold text-rose-700">This action cannot be undone. All sources and annotations will be permanently deleted.</p>
              <div className="flex gap-2">
                <button type="button" onClick={handleDeleteVault} className="flex-1 py-1.5 text-xs border-2 border-rose-500 bg-rose-500 text-white font-black font-display rounded cursor-pointer">Confirm Delete</button>
                <button type="button" onClick={() => setConfirmDeleteVault(false)} className="flex-1 py-1.5 text-xs border-2 border-neo-dark bg-stone-50 font-black font-display rounded cursor-pointer">Cancel</button>
              </div>
            </div>
          )}
        </div>
